      ...
      return {
        ...
        controller: [
          ...
          function(
            ...
            $scope.countdown =
              angular.isNumber($scope.countdownattr) &&
            ...
            $scope.start = function() {
              $scope.startTime = $scope.startTimeAttr
                ? moment($scope.startTimeAttr)
                : moment();
              $scope.endTime = $scope.endTimeAttr
                ? moment($scope.endTimeAttr)
                : null;
            ...
            $scope.resume = function() {
              ...
              $scope.startTime = moment().diff(
                moment($scope.stoppedTime).diff(moment($scope.startTime))
              );
              tick();
              $scope.isRunning = true;
              $scope.$emit("timer-started", {
            ...
            function calculateTimeUnits() {
              ...
              if (!$scope.maxTimeUnit || $scope.maxTimeUnit === "day") {
                ...
                $scope.hours = Math.floor(($scope.millis / 3600000) % 24);
                $scope.days = Math.floor(($scope.millis / 3600000 / 24) % 30);
                $scope.months = Math.floor(
                  ($scope.millis / 3600000 / 24 / 30) % 12
                );
              ...
              $scope.hhours =
                $scope.hours < 10 ? "0" + $scope.hours : $scope.hours;
              $scope.ddays = $scope.days < 10 ? "0" + $scope.days : $scope.days;
              $scope.mmonths =
                $scope.months < 10 ? "0" + $scope.months : $scope.months;
              $scope.yyears =
                $scope.years < 10 ? "0" + $scope.years : $scope.years;
            }
            ...
            if ($scope.countdownattr) {
              ...
              $scope.$on("timer-set-countdown-seconds", function(
                e,
                countdownSeconds
              ) {
                if (!$scope.isRunning) {
            ...
            var tick = function tick() {
              ...
              if (typeTimer !== null) {
                ...
                $scope.progressBar = progressBarService.calculateProgressBar(
                  $scope.startTime,
                  $scope.millis,
                  $scope.endTime,
                  $scope.countdownattr
  ...
  I18nService.prototype.getTimeUnits = function getTimeUnits(millis) {
    ...
    if (typeof this.timeHumanizer != "undefined") {
      time = {
        ...
        days: this.timeHumanizer(diffFromAlarm, {
          units: ["d", "h", "m", "s"]
        }),
        months: this.timeHumanizer(diffFromAlarm, {
          units: ["mo", "d", "h", "m", "s"]
        }),
        years: this.timeHumanizer(diffFromAlarm, {
          units: ["y", "mo", "d", "h", "m", "s"]
        })
      };
    } else {
      console.log(
        "esta madre fallo por algo, espero que no se por lo que puse"
      );

      console.error(
        'i18nService has not been initialized. You must call i18nService.init("en") for example'
      );
    }
...
app.factory("progressBarService", function() {
   ...
   * @return {float} 0 --> 100
   */
  ProgressBarService.prototype.calculateProgressBar = function calculateProgressBar(
    startValue,
    remainingTime,
    endTimeAttr,
    coutdown
